...
export default function RoomPage() {
 ...
 }, [cleanup, iceServers, peerId, roomId, signalingUrl]);
 ...
 {!camOn && <div className="absolute inset-0 flex items-center justify-center text-xs text-[var(--brand-text-muted)]">Cam OFF</div>}
 </div>
 <div className="mt-1 flex items-center justify-between">
 <p className="truncate text-[11px]">あなた</p>
 <span className={`text-[10px] ${micOn ? "text-[var(--brand-primary)]" : "text-[var(--brand-accent)]"}`}>{micOn ? "🎤" : "🔇"}</span>
 </div>
 </div>
 ...
 className="rounded-lg px-3 py-2 text-xs font-medium text-[var(--brand-text)] transition-colors "
 >
 {copied ? "✅ コピー済み" : "🔗 招待リンク"}
 </button>
 <button
 ...
 <p className={`mb-1 text-[11px] font-semibold ${message.mine ? "text-[var(--brand-primary)]" : "text-[var(--brand-primary)]"}`}>{message.user}</p>
