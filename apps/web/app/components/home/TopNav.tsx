              ...
              return (
                <div key={item.href ?? item.label} className="flex items-center">
                  {item.href ? (
                    <Link
                      href={item.href}
                      className={`px-5 py-2 transition-colors ${isActive ? "font-medium text-[#1e3a5f]" : "text-gray-600 hover:text-gray-900"}`}
                    >
                      {item.label}
                    </Link>
                  ) : (
                    <span className="px-5 py-2 text-gray-600">{item.label}</span>
                  )}
                  {index < NAV_ITEMS.length - 1 && <span className="text-gray-300">|</span>}
                </div>
              );
            })}
